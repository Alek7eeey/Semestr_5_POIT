...
-- 1.	Разработайте простейший анонимный блок PL/SQL (АБ),
-- не содержащий операторов.
      DECLARE
        BEGIN
          NULL;
        END;
...
-- 4.Разработайте вложенный блок.
-- Продемонстрируйте принцип обработки исключений во вложенных блоках.
...
BEGIN
  ...
  dbms_output.put_line('This is the end of inner block');
end;

-- 5.Выясните, какие типы предупреждения компилятора поддерживаются
-- в данный момент.
...
-- 8.Разработайте скрипт, позволяющий просмотреть все параметры Oracle Server,
-- связанные с PL/SQL.
...
-- 11.	арифметические действия над двумя
-- целыми number-переменных, включая деление с остатком;
...
DECLARE
  v_num1 NUMBER := 1.1;
  v_num2 NUMBER(3, 1) := 2.2; --3 - общее, 1 - количество дробных
BEGIN
  DBMS_OUTPUT.PUT_LINE(v_num1);
  DBMS_OUTPUT.PUT_LINE(v_num2);
END;

-- 13.	объявление и инициализацию number-переменных с фиксированной
-- точкой и отрицательным масштабом (округление);
DECLARE
  v_num1 NUMBER := 1.1;
  v_num2 NUMBER(3, -1) := 22.6;
BEGIN
  DBMS_OUTPUT.PUT_LINE(v_num1);
  DBMS_OUTPUT.PUT_LINE(v_num2);
END;
...
-- 16.	объявление number-переменных с точкой и применением
-- символа E (степень 10) при инициализации/присвоении;
DECLARE
  v_num1 NUMBER := 1.1E1;
  v_num2 NUMBER := 1.1E-1;
  v_num3 NUMBER := v_num1 + v_num2;
BEGIN
  DBMS_OUTPUT.PUT_LINE(v_num1);
  DBMS_OUTPUT.PUT_LINE(v_num2);
...
-- 18.	Разработайте анонимный блок PL/SQL содержащий
-- объявление констант (VARCHAR2, CHAR, NUMBER).
